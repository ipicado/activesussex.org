...
class JSecurImage {
	...
	*/
	...
	function JSecurImage()
	{
		...
		$this->wordlist_file = JPATH_SITE.DS.'components'.DS.'com_rscomments'.DS.'helpers'.DS.'securimage'.DS.'words'.DS.'words.txt';
		$this->use_wordlist  = false;

		$this->gd_font_file  = JPATH_SITE.DS.'components'.DS.'com_rscomments'.DS.'helpers'.DS.'securimage'.DS.'gdfonts'.DS.'automatic.gdf';
		$this->use_gd_font   = false;
		$this->gd_font_size  = 24;
		$this->text_x_start  = 15;

		$this->ttf_file      = JPATH_SITE.DS.'components'.DS.'com_rscomments'.DS.'helpers'.DS.'securimage'.DS.'AHGBold.ttf';
		...
		$this->signature_font  = JPATH_SITE.DS.'components'.DS.'com_rscomments'.DS.'helpers'.DS.'securimage'.DS.'AHGBold.ttf';

		$this->audio_path   = JPATH_SITE.DS.'components'.DS.'com_rscomments'.DS.'helpers'.DS.'securimage'.DS.'audio'.DS;
	...
	function saveData()
	{
		...
		$session->set('com_rscomments.securimage.value', $this->code);
		$session->set('com_rscomments.securimage.ctime', time());
	}
	...
	function validate()
	{
		...
		if ($code != '' && $code == $code_entered)
		{
			...
			$session->set('com_rscomments.securimage.value', '');
			$session->set('com_rscomments.securimage.ctime', '');
		}
	}
	...
	{
		$session = JFactory::getSession();
		return $session->set('com_rscomments.securimage.value', '');
	}
	...
	{
		$session = JFactory::getSession();
		$creation_time = $session->get('com_rscomments.securimage.ctime', 0);
		$expired = true;
